...
`@jaemin/eslint-config` is a public package and lint harness. It already has a focused `Check` workflow that runs `pnpm check`, which includes typecheck, lint, Vitest, and build. Prior planning explicitly favored small CI-only hardening before publish prep, and the package still benefits from more dogfood evidence before tightening defaults or adding publish automation.

## Proposed Automation

Add Labeler:

- `area:rules` for `src/rules/**` and `docs/rules/**`.
- `area:configs` for `src/configs/**`.
- `area:docs` for `docs/**` and `README.md`.
- `area:tests` for `test/**`, `*.test.ts`, and Vitest-related files.
- `area:ci` for `.github/**`.
- `type:feature`, `type:fix`, `type:docs`, and `type:chore` from branch naming patterns when reliable.

Add Release Drafter as a draft-only changelog assistant:
...
- Keep stale issue handling off unless issue volume grows.
- If added later, exempt dogfood reports and design discussions from auto-close.

## Explicit Non-Goals

- Do not add Super-Linter. This repository is itself an ESLint config harness, and `pnpm check` is the authoritative validation contract.
- Do not add npm publish automation in this phase.
- Do not change rule defaults, package metadata, or dogfood strategy.

## Testing Plan

- Preserve the existing `Check` workflow unchanged.
- Validate the new Labeler and Release Drafter YAML/config files syntactically.
- Confirm Release Drafter creates or updates only a GitHub draft release.
